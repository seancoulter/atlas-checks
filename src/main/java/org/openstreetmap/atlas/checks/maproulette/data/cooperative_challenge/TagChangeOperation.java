 ...
 * Reference: https://github.com/osmlab/maproulette3/wiki/Cooperative-Challenges#creating-tag-fix-challenges
 *
 * @author seancoulter
 */
public class TagChangeOperation extends CooperativeChallengeOperation
{
    ...
    /**
     * Using fields from the {@link ChangeDescription} associated with this {@link TagChangeOperation}, create the operation it stands for
     *
    ...
    public TagChangeOperation create()
    {
        ...
        this.getChangeDescriptorList().forEach(changeDescriptor ->
        {
            ...
            if (UNSET_TAGS.equals(action))
            {
                ...
                tagChanges.add(new JsonPrimitive(((TagChangeDescriptor) changeDescriptor).getKey()));
                nestedOperation.add(DATA_KEY, tagChanges);
            }
            else if (SET_TAGS.equals(action))
            {
                final JsonObject tagChanges = new JsonObject();
                tagChanges.add(((TagChangeDescriptor) changeDescriptor).getKey(), new JsonPrimitive(((TagChangeDescriptor) changeDescriptor).getValue()));
                nestedOperation.add(DATA_KEY, tagChanges);
            }
     ...
     *
     * @param descriptorType the {@link ChangeDescriptorType}
     * @return the cooperative challenge translation
     */
    private String convertAction(final ChangeDescriptorType descriptorType)
    {
        if (descriptorType.equals(ChangeDescriptorType.ADD) || descriptorType.equals(ChangeDescriptorType.UPDATE))
        {
